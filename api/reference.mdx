...
| `PATCH` | `/v1/workflows/{id}` | contributor+ | Update draft graph |
...
`params` become workflow **input** (`{{input.*}}` in steps).
...
| `GET` | `/v1/billing/plans` | contributor+ | Plan catalog (Stripe-synced) |
