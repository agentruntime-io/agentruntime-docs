...
## Multi-product catalog (upcoming)

AgentRuntime is extending the billing catalog so plans and Stripe prices can be scoped by **product** (for example, Console vs future product lines). Today, each workspace has a single plan and credit balance.

When multi-product billing ships:

- Plan codes may include a `product_code` dimension in API responses
- Self-serve checkout in **Settings → Plans** will filter to the active product for your Console deployment
- Existing workspaces keep their current plan during migration — no action required

Until then, treat **Settings → Plans** and `GET /v1/billing/plans` as the source of truth for your workspace.
