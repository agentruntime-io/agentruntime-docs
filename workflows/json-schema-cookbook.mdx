...
For editing graphs in the Console, see [JSON editor](/workflows/json-editor) (Canvas \| JSON mode, IntelliSense, **Apply to canvas**).
...
```json
{
  ...
  "type": "for_each",
  "name": "Triage tickets",
  "depends_on": ["load-tickets"],
  "for_each_items": "{{steps.load-tickets.result.tickets}}",
  ...
  "for_each_body_steps": [
    {
      ...
      "prompt": "Triage: {{foreach.item}}"
    },
    {
      "id": "tag",
      "type": "lua_script",
      "depends_on": ["triage"],
      "script": "return { ticket = tostring(item), ok = true }"
    }
  ]
}
```
...
```json
{
  "id": "refine",
  "type": "while",
  "name": "Refine until score passes",
  "while_mode": "until",
  "while_max_iterations": 5,
  "while_condition": "{{body.score.pass}}",
  "while_body_steps": [
    {
      "id": "draft",
      "type": "lua_script",
      "script": "local n = loop.iteration or 0\nreturn { text = string.rep('x', (n+1)*40) }"
    },
    {
      "id": "score",
      "type": "lua_script",
      "depends_on": ["draft"],
      "script": "local n = #tostring(body.draft.text or '')\nreturn { pass = n >= 90 }"
    }
  ]
}
```
...
| `{{steps.step-id.result.field}}` | Upstream step output (after the step completes) |
| `{{steps.step-id.input.field}}` | Resolved parameters the step was called with (after template expansion) |
| `{{foreach.item}}` | Current item in `for_each` body |
| `{{foreach.index}}` | Zero-based index in `for_each` body |
| `{{body.inner-id.field}}` | Inner step output in `while` / `for_each_body_steps` (current iteration) |
| `{{while.iteration}}` | Current while iteration (0-based) |
| `{{while.prev.inner-id.field}}` | Previous while iteration inner output |

**Composite loops:** Inner body steps also expose `{{steps.<inner-id>.input.<field>}}` using the inner step `id` from `for_each_body_steps` or `while_body_steps`. Parent loop nodes expose their own resolved inputs, e.g. `{{steps.process-each.input.for_each_items}}` or `{{steps.refine.input.while_condition}}`.

**Legacy shapes:** Do not use `for_each_body`, slug step ids, or catalog-agent-only canvas nodes.
