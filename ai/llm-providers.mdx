...
Workspace keys are stored in Vault. After saving, you can reveal or rotate secrets from the same card. Workflow **llm_call** steps resolve models against this catalog — you pick a `model` ID in Workflow Studio; the runtime chooses the matching provider key.
...
<Steps>
  ...
  <Step title="Save and verify">
    Save the key, then confirm models for that provider appear in the catalog. In Workflow Studio, add an **llm_call** step and set `model` to a catalog ID (for example `gpt-4o`).
  </Step>
...
In Workflow Studio, add an **llm_call** step and set:

| Field | Example |
|-------|---------|
| `model` | `gpt-4o`, `claude-sonnet-4-20250514`, or catalog ID |
| `prompt` | Instructions with `{{steps.*}}` template variables |
| `llm_params` | `temperature`, `max_tokens`, etc. |

The runtime resolves the provider from the model ID and uses the bound API key.
