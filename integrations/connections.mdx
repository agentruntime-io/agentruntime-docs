...
Connections are separate from workflow graphs. You **wire** them to MCP instances through **config profiles** on each instance (`/mcp/instances/{instance_id}`), then reference `instance_id` in workflow **mcp_call** steps.
