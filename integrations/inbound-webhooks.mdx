...
4. The BFF verifies the signature and starts a run with the POST body as workflow `input`
...
The POST body becomes workflow trigger **input**. Reference fields in steps with `{{input.field_name}}`.
