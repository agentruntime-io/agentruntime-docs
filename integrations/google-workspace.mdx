...
| Works in dev, fails in prod | Use a separate MCP instance wired to the prod connection, or duplicate the workflow with the correct `instance_id` on MCP steps |
